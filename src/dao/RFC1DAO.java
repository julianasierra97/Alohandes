...
public class RFC1DAO 
{
	...
	public String RFC1() throws SQLException, Exception {
		...
		String elAnio= fechaActual.getYear()+"";
		...
		String sql2 = String.format("SELECT SUM(CONTRATO.COSTO) as GananciaActualVivienda, %1$s.VIVIENDA.ID_PERSONA as ID FROM %1$s.CONTRATO  INNER JOIN %1$s.VIVIENDA ON %1$s.CONTRATO.ID_VIVIENDA= VIVIENDA.ID WHERE %1$s.CONTRATO.FECHAFIN< '31/12/%2$s' AND %1$s.CONTRATO.FECHAINICIO>'01/01/%2$s' AND %1$s.CONTRATO.ESTADO='Activo' GROUP BY %1$s.VIVIENDA.ID_PERSONA " ,
				USUARIO,
				elAnio);
		...
		String elAnioPasado= fechaActual.getYear()-1+"";

		String sql3 = String.format("SELECT SUM(COSTO) as GananciaActualHabitacion,  %1$s.OPERADORHABITACION.ID_OPERADOR as ID FROM %1$s.CONTRATO  INNER JOIN %1$s.OPERADORHABITACION ON %1$s.CONTRATO.ID_HABITACION= %1$s.OPERADORHABITACION.ID  WHERE %1$s.CONTRATO.FECHAFIN< '31/12/%2$s' AND %1$s.CONTRATO.FECHAINICIO>'01/01/%2$s' AND %1$s.CONTRATO.ESTADO='Activo' GROUP BY %1$s.OPERADORHABITACION.ID_OPERADOR " ,
				USUARIO,
				elAnio);
